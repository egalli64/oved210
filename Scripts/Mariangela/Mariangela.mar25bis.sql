...
alter table simple
add constraint simple_coders_id_fk
...
insert into coders
values(12, 'James', 'Bond', sysdate, 4000);
...
alter table simple
add constraint simple_code_id_fk
coder_id references coders(coder_id) on delete set null; -- inserisce null

alter table simple
add constraint simple_code_id_fk
coder_id references coders(coder_id) on delete cascade; -- elimina tutto
...
-- !!!
truncate table my_statuses; -- cancella tutti i valori inseriti non puoi fare il rollback 

drop table my_statuses; -- cancella la tabella 

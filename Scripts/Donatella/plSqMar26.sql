...
'is' || v_cur.first_name||''||v_cur.last_name);
end loop;
end;
/
--open cursor for
...
declare
type t_coder_cursor is ref cursor ;
cursor t_cursor;
...
loop
  ...
  exit when v_cursor%notfound;
     DBMS_OUTPUT.PUT_LINE('Coder'  || v_cod.coder_id ||
'is' || v_cod.first_name||''||v_cod.last_name);
end loop;

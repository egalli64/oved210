...
public class HashesExamples {
	public static void hashSetExample() {
		...
		hs2.addAll(myStrings);   // se ci fossero dei dupplicati, andrebbero persi
		hs2.clear();
		System.out.println(myStrings + ", " + hs2);

		HashSet<String> hs3 = new HashSet<>(myStrings);
		boolean al3HasFirst = hs3.contains("First");   // ci permette di vedere se esiste o non esiste qst elemento
...
//		System.out.println(al3.lastIndexOf("Second"));
		...
		Iterator<String> it = hs3.iterator();    // per fare il loop sugli elementi bisogna fare il for each :
		while(it.hasNext()) {
